...
connection_pools: Dict[Tuple[Tuple[str, Any], ...], SimpleConnectionPool] = {}


def _pool_key(config: Dict[str, Any]) -> Tuple[Tuple[str, Any], ...]:
    """Generate a hashable key for a PostgreSQL configuration dictionary."""

    return tuple(sorted(config.items()))


@contextmanager
def get_pg_connection(config: Dict[str, Any]):
    """Return a PostgreSQL connection from a shared pool."""

    key = _pool_key(config)
    pool = connection_pools.get(key)
    if pool is None:
...
def close_connection_pools() -> None:
    """Close all active PostgreSQL connection pools."""
...
def cargar_consulta(archivo):
    ...
    print(query)
    try:
        with get_pg_connection(pgsql_config) as connlp:
            with connlp.cursor() as respuesta:
...
def ejecutarpaso(proceso, panel_config):
    ...
    try:
        ejecutarpaso = False
        with get_pg_connection(panel_config) as connlp:
            with connlp.cursor() as horaaut:
...
def registrar_paso(proceso, codigo_proceso, panel_config):
    try:
        with get_pg_connection(panel_config) as connrp:
...
def procesar_e_insertar(pgsql_config, panel_config, test, query_sql):
    ...
    try:
        ...
        errores = []
        actualizar = False
        with get_pg_connection(pgsql_config) as conn:
            for fila in rows:
                try:
...
def grabarcedencedulasconqr(pmovimientoid, pactuacionid, pdomicilioelectronicopj, sgdocid, pgsql_config):
    ...
    try:
        with get_pg_connection(pgsql_config) as connrp:
            with connrp.cursor() as cursor:
                ahora = datetime.now()
                query = f"""
...
def registrar_error(dbgusername, dbguservalor, dbguserprograma):
    ...
    try:
        ...
        print(f"Error {query}")
        with get_pg_connection(panel_test) as connpanel:
            with connpanel.cursor() as logserror:
...
def ejecutar_control_cedulas(pgsql_config, pmovimientoid, pactuacionid, pdomicilioelectronicopj):
    try:
        with get_pg_connection(pgsql_config) as conexion:
